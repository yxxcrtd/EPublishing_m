(function(w) {
    ...
    a.init = function(b, c) {
        ...
        if (y)
            o = p[0], this.can = n("canvas", 
            this.cont), this.con = this.can.getContext("2d"), this.cCan = m(n("canvas", this.cont), {display: "none"}), this.cCon = this.cCan.getContext("2d");
        else {
            o = p[1];
...
(function(factory) {
    ...
    $.smodal.defaults = {appendTo: "body",focus: true,opacity: 50,overlayId: "simplemodal-overlay",overlayCss: {},containerId: "simplemodal-container",
    ...
    $.smodal.impl = {d: {},init: function(data, options) {
            var s = this;
            if (s.d.data)
                return false;
    ...
    $.fn.doubletap = function(onDoubleTapCallback, onTapCallback, delay) {
        ...
        $(this).bind(eventName, function(event) {
            ...
            if (delta < delay && delta > 0) {
                ...
                null && typeof onDoubleTapCallback == "function")
    ...
    $.fn.touchwipe = function(settings) {
        ...
        this.each(function() {
            ...
            function onTouchMove(e) {
                ...
                isMoving)
                    if (!(jQuery(e.target).hasClass("flexpaper_note_textarea") || jQuery(e.target).hasClass("flexpaper_textarea_data")))
                        e.preventDefault();
...
(function(h) {
    ...
    function(a, b) {
        ...
        if ("-webkit-" == s) {
            ...
            c = 
            {x: c.x / 100 * a.width(),y: c.y / 100 * a.height()};
            var D = c.x - b.x;
            f = c.y - b.y;
            var h = Math.atan2(f, D), k = h - Math.PI / 2, k = Math.abs(a.width() * Math.sin(k)) + Math.abs(a.height() * Math.cos(k)), D = Math.sqrt(f * f + D * D);
            ...
            for (f = 0; f < e; f++)
                G.push(" " + d[f][1] + " " + 100 * (b + D * d[f][0]) / k + "%");
            a.css({"background-image": s + "linear-gradient(" + -h + "rad," + G.join(",") + ")"})
        }
    }, R = "ontouchstart" in 
    window ? {down: "touchstart",move: "touchmove",up: "touchend",over: "touchstart",out: "touchend"} : {down: "mousedown",move: "mousemove",up: "mouseup",over: "mouseover",out: "mouseout"}, g = {init: function(a) {
            void 0 === A && (A = "WebKitCSSMatrix" in window || "MozPerspective" in document.body.style, s = W());
            var b, c = this.data(), d = this.children();
            a = h.extend({width: this.width(),height: this.height()}, U, a);
            c.opts = a;
            c.pageObjs = {};
            c.pages = {};
            c.pageWrap = {};
            ...
            c.eventHandlers = {move: function(a) {
                    for (var b in c.pages)
                        q(b, c.pages) && k._eventMove.call(c.pages[b], a)
                },end: function(a) {
                    for (var b in c.pages)
                        q(b, c.pages) && k._eventEnd.call(c.pages[b], a)
                },start: function(a) {
                    for (var b in c.pages)
                        if (q(b, 
                        c.pages) && !1 === k._eventStart.call(c.pages[b], a))
                            return !1
                }};
            ...
            if (c)
                if (g._necessPage.call(this, a)) {
                    if (!b.pageWrap[a]) {
                        ...
                        b.pagePlace[a] = a;
                        b.pageWrap[a] = h("<div/>", {id: "turn-page-wrapper-" + a,"class": "turn-page-wrapper",
                            page: a,css: {position: "absolute",overflow: "hidden",width: d,height: e}}).css(O["double" == b.display ? a % 2 : 0]);
                        this.append(b.pageWrap[a]);
                        b.pageObjs[a].appendTo(b.pageWrap[a])
                    }
                    (!a || 1 == g._setPageLoc.call(this, a)) && g._makeFlip.call(this, a)
                } else
                    b.pagePlace[a] = 0, b.pageObjs[a] && b.pageObjs[a].detach()
        },hasPage: function(a) {
            ...
            var c = b.totalPages;
            if (b.eventHandlers)
                for (jQuery(document).unbind(R.move, b.eventHandlers.move).unbind(R.up, b.eventHandlers.end); 0 !== c; )
                    this.turn("removePage", c), c -= 1;
            ...
            if (!b.pages[a] && b.pagePlace[a] == a) {
                ...
                b.pages[a] = b.pageObjs[a].css({width: c ? this.width() : this.width() / 
                    2,height: this.height()}).flip({page: a,next: c && a === b.totalPages ? a - 1 : d || c ? a + 1 : a - 1,turn: this,duration: b.opts.duration,acceleration: b.opts.acceleration,corners: c ? "all" : d ? "forward" : "backward",backGradient: b.opts.gradients,frontGradient: b.opts.gradients,cornerDragging: b.opts.cornerDragging}).flip("disable", b.disabled).bind("pressed", g._pressed).bind("released", g._released).bind("start", g._start).bind("end", g._end).bind("flip", g._flip)
            }
            return b.pages[a]
            ...
            if (!(1 > a || a > d.totalPages))
                return e[1] = e[1] || e[0], 1 <= e[0] && e[1] <= d.totalPages ? (a = Math.floor(9), d.totalPages - e[1] > e[0] ? (b = Math.min(e[0] - 1, a), c = 2 * a - b) : (c = Math.min(d.totalPages - e[1], a), b = 2 * a - c)) : c = b = 19, [Math.max(1, e[0] - b), Math.min(d.totalPages, e[1] + c)]
            ...
            var c, d = this.data(), e = "single" == d.display, f = function(a) {
                ...
                {page: c,next: e || f ? c + 1 : c - 1,corners: e ? "all" : f ? "forward" : "backward"})), b && (delete d.pages[a], delete d.pagePlace[a], delete d.pageObjs[a], delete d.pageWrap[a], delete d.pageObjs[a]))
            };
            if (0 < b)
            ...
            if (a) {
                ...
                "single" == a ? b.pageObjs[0] || (this.turn("stop").css({overflow: "hidden"}), b.pageObjs[0] = h("<div />", {"class": "turn-page p-temporal"}).css({width: this.width(),
                    height: this.height(),"background-color": "#ffffff"}).appendTo(this)) : b.pageObjs[0] && (this.turn("stop").css({overflow: ""}), b.pageObjs[0].detach());
                b.display = a;
            ...
            var a, b = this.data();
            for (a in b.pages)
                if (q(a, b.pages) && b.pages[a].data().f.hoveringCorner)
                    return !0;
            return !1
        },disable: function(a) {
            var b, c = 
            this.data(), d = this.turn("view");
            c.disabled = void 0 === a || !0 === a;
            for (b in c.pages)
                q(b, c.pages) && c.pages[b].flip("disable", a ? h.inArray(b, d) : !1);
            return this
            ...
            for (b in c.pages)
                q(b, c.pages) && (c.pages[b].data().f.opts.cornerDragging = a)
        },size: function(a, b, c) {
            if (a && b) {
                this.turn("stop");
                var d = this.data(), e = "double" == d.display ? a / 2 : a, f;
                if (c)
                    for (f in this.css({width: a,height: b}), d.pageObjs[0] && d.pageObjs[0].css({width: e,
                        height: b}), d.pageWrap)
                        q(f, d.pageWrap) && (d.pageWrap[f].css({width: e,height: b}), d.pageObjs[f].width(), d.pageObjs[f].animate({width: e,height: b}, {duration: 700,step: function(a, b) {
                            }}), d.pages[f] && d.pages[f].css({width: e,height: b}));
                else
                    for (f in this.css({width: a,height: b}), d.pageObjs[0] && d.pageObjs[0].css({width: e,height: b}), d.pageWrap)
                        q(f, d.pageWrap) && (d.pageObjs[f].css({width: e,height: b}), d.pageWrap[f].css({width: e,height: b}), d.pages[f] && d.pages[f].css({width: e,height: b}));
                this.turn("resize");
                return this
            }
            ...
            var b, c = this.data(), d = c.pageMv;
            c.pageMv = [];
            c.tpage && (c.page = c.tpage, delete c.tpage);
            for (b in d)
                q(b, d) && (k._moveFoldingPage.call(c.pages[d[b]], null), k.hideFoldedPage.call(c.pages[d[b]], !1), a = c.pages[d[b]].data().f.opts, c.pagePlace[a.next] = a.next, a.force && (a.next = 0 === a.page % 2 ? a.page - 1 : a.page + 
                1, delete a.force));
            this.turn("update");
            return this
        },pages: function(a) {
            ...
            var b = this.data(), c = this.turn("range", a), d = [];
            for (a = c[0]; a <= c[1]; a++)
            ...
            var a = h(this).data().f.opts, b = a.turn, c = b.data();
            a.pageMv = a.page;
            ...
            if (b || e.tpage) {
                if (e.tpage == c.next || e.tpage == c.page)
                    delete e.tpage, g._fitPage.call(d, e.tpage || 
                    c.next, !0)
            } else
                z && g._removeMv.call(d, c.pageMv), d.turn("update")
        },_pressed: function() {
            var a = h(this).data().f;
            a.opts.turn.data();
        ...
        _flip: function() {
            ...
            var b, c, d, e, f = this, h = this.data();
            b = this.turn("view");
            var k = b[0] || b[1], g = {pageZ: {},partZ: {},pageV: {}}, l = function(a) {
                a = f.turn("view", a);
                a[0] && (g.pageV[a[0]] = !0);
                a[1] && (g.pageV[a[1]] = !0)
            };
            for (b = 0; b < a.length; b++)
                c = a[b], d = h.pages[c].data().f.opts.next, e = h.pagePlace[c], l(c), l(d), c = h.pagePlace[d] == d ? d : c, g.pageZ[c] = h.totalPages - Math.abs(k - c), g.partZ[e] = 2 * h.totalPages + Math.abs(k - c);
            return g
        },update: function() {
            var a, 
            b = this.data();
            if (b.pageMv.length && 0 !== b.pageMv[0]) {
                ...
                for (a in b.pageWrap)
                    q(a, b.pageWrap) && (b.pageWrap[a].css({display: c.pageV[a] ? "" : "none","z-index": c.pageZ[a] || 0}), b.pages[a] && (b.pages[a].flip("z", c.partZ[a] || null), c.pageV[a] && b.pages[a].flip("resize"), b.tpage && b.pages[a].flip("disable", !0)))
            } else
                for (a in b.pageWrap)
                    q(a, b.pageWrap) && (c = g._setPageLoc.call(this, a), b.pages[a] && b.pages[a].flip("disable", b.disabled || 1 != c).flip("z", null))
        },
        _updateShadow: function() {
            var a = this.data(), b = this.turn("view"), c = this.width(), d = this.height(), e = "single" == a.display ? c : c / 2;
            a.shadow || (a.shadow = h("<div />", {"class": "flexpaper_shadow",css: m(0, 0, 0).css}).appendTo(this));
            b[0] == a.totalPages && 0 == a.totalPages % 2 ? a.shadow.css({width: e,height: d,top: 0,left: 0}) : 0 == b[0] ? a.shadow.css({width: e,height: d,top: 0,left: e}) : a.shadow.css({width: c,height: d,top: 0,left: 0})
        },_setPageLoc: function(a) {
            var b = this.data(), c = this.turn("view");
            if (a == c[0] || a == c[1])
                return b.pageWrap[a].css({"z-index": b.totalPages,
                    display: ""}), 1;
            ...
            {opts: h.extend({}, b.opts || V, a)}), this) : b.opts
        },z: function(a) {
            var b = this.data().f;
            b.opts["z-index"] = a;
            ...
            var b = this.data().f, c = b.parent.offset(), d = this.width(), e = this.height();
            ...
            b = b || 0;
            return {tl: l(b, b),tr: l(c - b, b),bl: l(b, d - b),br: l(c - b, d - b)}[a]
        },_c2: function(a) {
            var b = this.width(), c = this.height();
            return {tl: l(2 * b, 0),tr: l(-b, 0),
                bl: l(2 * b, c),br: l(-b, c)}[a]
            ...
            if ((b = a.opts.backGradient && (!b || "single" == b.data().display || 2 != a.opts.page && a.opts.page != b.data().totalPages - 1)) && !a.bshadow)
                a.bshadow = h("<div/>", m(0, 0, 1)).css({position: "",width: this.width(),height: this.height()}).appendTo(a.parent);
            return b
        },resize: function(a) {
            var b = this.data().f, c = this.width(), d = this.height(), e = Math.round(Math.sqrt(Math.pow(c, 2) + Math.pow(d, 2)));
            a && (b.wrapper.css({width: e,height: e}), b.fwrapper.css({width: e,height: e}).children(":first-child").css({width: c,height: d}), b.fpage.css({width: d,height: c}), b.opts.frontGradient && b.ashadow.css({width: d,height: c}), k._backGradient.call(this) && b.bshadow.css({width: c,height: d}));
            b.parent.is(":visible") && (b.fwrapper.css({top: b.parent.offset().top,left: b.parent.offset().left}), 
            ...
            if (!a.wrapper) {
                ...
                a.fparent || (c = h("<div/>", {css: {"pointer-events": "none"}}).hide(), c.data().flips = 0, a.opts.turn ? (c.css(m(-a.opts.turn.offset().top, 
                -a.opts.turn.offset().left, "auto", "visible").css).appendTo(a.opts.turn), a.opts.turn.data().fparent = c) : c.css(m(0, 0, "auto", "visible").css).attr("id", "turn-fwrappers").appendTo(h("body")), a.fparent = c);
                ...
                a.fpage = h("<div/>", {css: {cursor: "default"}}).appendTo(h("<div/>", m(0, 0, 0, 
            ...
            var b = this, c = 0, d = 0, e, f, h, g, x, q, B = l(0, 0), N = l(0, 0), n = l(0, 0), t = this.width(), w = this.height(), m = k._foldingPage.call(this);
            Math.tan(d);
            var r = this.data().f, y = r.opts.acceleration, z = r.wrapper.height(), 
            s = k._c.call(this, a.corner), H = "t" == a.corner.substr(0, 1), E = "l" == a.corner.substr(1, 1), J = function() {
                var p = l(s.x ? s.x - a.x : a.x, s.y ? s.y - a.y : a.y), F = Math.atan2(p.y, p.x), C;
                d = I - F;
                c = 180 * (d / K);
                C = l(E ? t - p.x / 2 : a.x + p.x / 2, p.y / 2);
                var m = d - Math.atan2(C.y, C.x), m = Math.max(0, Math.sin(m) * Math.sqrt(Math.pow(C.x, 2) + Math.pow(C.y, 2)));
                n = l(m * Math.sin(d), m * Math.cos(d));
                if (d > I && (n.x += Math.abs(n.y * Math.tan(F)), n.y = 0, Math.round(n.x * Math.tan(K - d)) < w))
                    return a.y = Math.sqrt(Math.pow(w, 2) + 2 * C.x * p.x), H && (a.y = w - a.y), J();
                d > I && (p = K - d, F = z - w / 
                ...
                var m = ["0", "auto"], x = (t - z) * e[0] / 100, s = (w - z) * e[1] / 100;
                c = {left: m[c[0]],top: m[c[1]],right: m[c[2]],bottom: m[c[3]]};
                m = 90 != n && -90 != n ? E ? -1 : 1 : 0;
                e = e[0] + "% " + e[1] + "%";
                b.css(c).transform(v(n) + u(a.x + m, a.y, y), e);
                r.fpage.parent().css(c);
                r.fpage.css({"-webkit-box-shadow": String.format("rgba(0, 0, 0, {0}) 0px 0px 20px", 0.25 < g ? g : 0.25),"-moz-box-shadow": String.format("rgba(0, 0, 0, {0}) 0px 0px 20px", 
                    0.25 < g ? g : 0.25),"-ms-box-shadow": String.format("rgba(0, 0, 0, {0}) 0px 0px 20px", 0.25 < g ? g : 0.25),"-o-box-shadow": String.format("rgba(0, 0, 0, {0}) 0px 0px 20px", 0.25 < g ? g : 0.25),"box-shadow": String.format("rgba(0, 0, 0, {0}) 0px 0px 20px", 0.25 < g ? g : 0.25),"-webkit-transition": "-webkit-box-shadow 2s","-moz-transition": "-moz-box-shadow 2s","-o-transition": "-webkit-box-shadow 2s","-ms-transition": "-ms-box-shadow 2s"});
                ...
                r.opts.frontGradient && Q(r.ashadow, l(E ? 100 : 0, H ? 100 : 0), l(f.x, f.y), [[g, "rgba(0,0,0,0)"], [0.8 * (1 - g) + g, "rgba(0,0,0," + 0.2 * q + ")"], [1, "rgba(255,255,255," + 0.2 * q + ")"]], 3, d);
                k._backGradient.call(b) && Q(r.bshadow, l(E ? 0 : 100, H ? 0 : 100), l(h.x, h.y), [[0.8, "rgba(0,0,0,0)"], [1, "rgba(0,0,0," + 0.3 * q + ")"], [1, "rgba(0,0,0,0)"]], 3)
            };
            switch (a.corner) {
                case "tl":
                    a.x = Math.max(a.x, 1);
            ...
            var b = this.data().f, c = k._foldingPage.call(this);
            if (c)
                if (a) {
                    if (!b.fpage.children()[b.ashadow ? "1" : "0"])
                        k.setData.call(this, {backParent: c.parent()}), b.fpage.prepend(c)
                } else
                    b.backParent && b.backParent.prepend(c)
        },_showFoldedPage: function(a, b) {
            var c = k._foldingPage.call(this), d = this.data(), e = d.f;
            ...
            if (c) {
                if (b) {
                    var g = this, c = e.point && e.point.corner == a.corner ? e.point : k._c.call(this, a.corner, 1);
                    this.animatef({from: [c.x, c.y],to: [a.x, a.y],duration: 500,frame: function(b) {
                            a.x = Math.round(b[0]);
                            a.y = Math.round(b[1]);
                            k._fold.call(g, a)
                        }})
                } else
                    k._fold.call(this, a), d.effect && !d.effect.turning && this.animatef(!1);
            ...
            if (b.point) {
                ...
                if (a) {
                    ...
                    var g = l(d.x, d.y + a), h = l(f.x, f.y - a);
                    this.animatef({from: 0,to: 1,frame: function(a) {
                            a = P(d, g, h, f, a);
                            d.x = a.x;
                            d.y = a.y;
                            k._fold.call(c, d)
            ...
            a = {corner: c.corner ? c.corner.corner : a || k._cAllowed.call(this)[0]};
            var d = c.point || k._c.call(this, a.corner, c.opts.turn ? c.opts.turn.data().opts.elevation : 
            0), e = k._c2.call(this, a.corner);
            this.trigger("flip").animatef({from: 0,to: 1,frame: function(c) {
                    c = P(d, d, e, e, c);
                    a.x = c.x;
                    a.y = c.y;
            ...
            if (!b.disabled && !this.flip("isTurning")) {
                ...
                if (b.corner && 
                k._foldingPage.call(this, b.corner))
                    return k._moveFoldingPage.call(this, !0), this.trigger("pressed", [b.point]), !1;
                b.corner = null
            }
            ...
            if (b && !b.disabled && b.opts.cornerDragging)
                if (a = z ? a.originalEvent.touches : [a], b.corner && !this.flip("isTurning")) {
                    ...
                    !this.data().effect && this.is(":visible") && ((a = k._cornerActivated.call(this, a[0])) && !this.flip("isTurning") ? 
            ...
            k.setData.call(this, {disabled: a});
            return this
        }}, S = function(a, b, c) {
        if (!c[0] || "object" == typeof c[0])
            return b.init.apply(a, c);
        if (b[c[0]] && 
        "_" != c[0].toString().substr(0, 1))
    ...
    h.extend(h.fn, {flip: function(a, b) {
            ...
            if (a) {
                ...
                var c, d = [], e = a.to.length, f = this, g = a.fps || 30, k = -g, l = !0, m = function() {
                    if (b.effect && l) {
                        var c, h = [];
                        k = Math.min(a.duration, k + g);
                        for (c = 0; c < e; c++)
                            h.push(a.easing(1, k, a.from[c], d[c], a.duration));
                        a.frame(1 == e ? h[0] : h);
                ...
                b.effect = h.extend({stop: function() {
                        l = !1
...
(function($) {
    ...
    {get: function(elem) {
            ...
            if (support.transform === "WebkitTransform" && !eb.browser.safari)
...
(function($) {
    $.fn.visible = function(partial, transitionSelector) {
        if (transitionSelector) {
            ...
            var $w = container, viewTop = translateY, viewBottom = viewTop + $w.height(), viewLeft = translateX, viewRight = viewLeft + $w.width();
