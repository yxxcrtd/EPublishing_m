					  ...
					  <div class="fr">
						<a title="${c.name}" href="${ctx}/pages/collection/form/list?id=${c.id}">
							<c:if test="${c.cover==null||c.cover=='' }">
								<img src="<c:if test="${ctx!=''}">${ctx}</c:if><c:if test="${ctx==''}">${domain}</c:if>/images/smallimg.jpg" width="52" height="71" onerror="this.src='${ctx}/images/smallimg.jpg'" class="imgbor mt5 mr10"/>
							</c:if>
							<c:if test="${c.cover!=null&&c.cover!='' }">
								<img  src="${ctx}${c.cover}" width="52" height="71" onerror="this.src='${ctx}/images/smallimg.jpg'" class="imgbor mt5 mr10"/>
							</c:if>	
						</a>	
					 </div>
					</div>	
