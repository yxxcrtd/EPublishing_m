...
<c:forEach items="${downList }" var="down" varStatus="status">
                ...
                <a href='${ctx }/index/search?type=2&isAccurate=1&searchValue="${down.publisher.name }"'>${down.publisher.name }</a>
                </c:if>
                <c:if test="${sessionScope.lang=='en_US'}">
                <a href='${ctx }/index/search?type=2&isAccurate=1&searchValue="${down.publisher.nameEn }"'>${down.publisher.nameEn }</a>
                </c:if>
