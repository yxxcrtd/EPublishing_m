               		...
               		if(marginBot<=2) {
               			if(curPage%5==0){
             				if($("#next"+curPage).val()=="0"){
             				 ...
             				 $("#freePubList").append('<div style="text-align: center;"><input type="hidden" id="next'+curPage+'" value="0" /><a href ="javascript:;" id="load'+curPage+'" onclick="loadNext();"  >加载更多</a></div>');
             				 return false;
             			}
               			$.ajax({
               				type : "GET",
               				async : false,    
               		        url: "${ctx}/mobile/pages/index/freePubJson",
               		        data: {	
               		        	curpage:curPage
               		        		},
               		            ...
               		            if(json.length>0){
               		            	$("#curPage").val(data.curpage);
               		             	for(var i=0;i<json.length;i++){
               		             		...
               		             		htmlcode+='<img width="14" height="14" src="${ctx}/images/ico/f.png" />';
               		             		if(json[i].type==1){
       			...
       			$.ajax({
       		        ...
       		        success : function(data) { 
       		            ...
       		            if(typeof(json)!='undefined' && json.length>0){
		 	            	...
		 	            	$("#freePubList").append('<div style="text-align: center;"><a href ="javascript:;" id="nomore">暂无更多数据</a></div>');
							return false;
       	             	}
       		       		//html全部拼完会出现一个undefined在最前面
    					if(htmlcode.indexOf("undefined")>=0){
...
<div data-role="page" data-theme="c" class="page" id="pageorder">
  ...
  <div data-role="content">
	<div data-role="header" class="classifyTit">
        <a class="back" href="javascript:window.location.href='${ctx }/index'"><img src="${ctx}/mobile/images/left.png"></a>
        <h1 class="ui-title-h1"><ingenta-tag:LanguageTag key="Global.Label.freePub" sessionKey="lang"/></h1>
    </div>
    <!-- 列表开始 -->
    <div class="order" id="freePubList">
