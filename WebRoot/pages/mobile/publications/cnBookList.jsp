    ...
    if(startY - y >50){
 		...
 		if(marginBot<=2) {
 			if(curPage%5==0){
 				if($("#next"+curPage).val()=="0"){
 				 ...
 				 $("#cnBookList").append('<div style="text-align: center;"><input type="hidden" id="next'+curPage+'" value="0" /><a href ="javascript:;" id="load'+curPage+'" onclick="loadNext();"  >加载更多</a></div>');
 				 return false;
 			}  
 			$.ajax({
 				type : "GET",
 		        ...
 		        success : function(data) { 
 		            ...
 		            if(typeof(json)!='undefined' && json.length>0){
	 	            	...
	 	            	$("#cnBookList").append('<div style="text-align: center;"><a href ="javascript:;" id="nomore">暂无更多数据</a></div>');
						return false;
 	             	}
		...
		try {  
			...
			function searchByCondition(type,value){
				...
				var lcense = $("#lcense").val();
				$("input[name='curpage']").val(0);
				$("#order2").val('');
				if(verify!=1){
					document.formList.action="${ctx}/mobile/pages/publications/lastPubsBook?isCn=true";
  ...
  <div data-role="content">
    ...
    <div data-role="navbar" class="navDiv">
        <ul>
          ...
          <li class="curLi"><a href="${ctx}/mobile/pages/publications/lastPubsBook?isCn=true" data-ajax="false">中文书</a></li>
        </ul>
  	</div>
    <!-- 中间内容部分开始 -->
    <div class="ui-grid-b resource">
        ...
        <div class="ui-block-b" style="width:35%;"><a href="javascript:;"onclick="searchByCondition('searchOrder','createOn')" >上架时间 <img id="ShelvesDate" src="${ctx}/mobile/images/icoDown.png" class="vm"></a></div>
     ...
     <div class="oh mb20" id="cnBookList">
    <c:forEach items="${list }" var="b">
      ...
      <!--以下 提交查询Form 开始-->
		<form:form action="${form.url}" method="post" modelAttribute="form" commandName="form" name="formList" id="formList">
			...
			<form:hidden path="lcense" id="lcense"/>
