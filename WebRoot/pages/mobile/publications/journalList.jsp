    ...
    if(startY - y >50){
 		...
 		if(marginBot<=2) {
 			if(curPage%5==0){
 				if($("#next"+curPage).val()=="0"){
 				 ...
 				 $("#journalList").append('<div style="text-align: center;"><input type="hidden" id="next'+curPage+'" value="0" /><a href ="javascript:;" id="load'+curPage+'" onclick="loadNext();"  >加载更多</a></div>');
 				 return false;
 			}
 			$.ajax({
 				type : "GET",
 				async : false,    
 		        url: "${ctx}/mobile/pages/publications/journalListJson",
 		       data: {	
 		    	  	curpage:curPage,
 		            	...
 		            	$("#curpage").val(data.curpage);
 		             	for(var i=0;i<json.length;i++){
						   ...
						   htmlcode+='<div class="fl wid85">';
 		                   ...
 		                   htmlcode+='<p>';
 		                   htmlcode+=json[i].publisher.name+'('+json[i].pubDate+')';
 		                   htmlcode+='</p>';
 		                   if(json[i].type ==2 &&  json[i].startVolume !=null && json[i].startVolume !="" && json[i].endVolume!=null && json[i].endVolume!=""){
 		                	  htmlcode+='<p>';
 	 	            	...
 	 	            	$("#journalList").append('<div style="text-align: center;"><a href ="javascript:;" id="nomore">暂无更多数据</a></div>');
 						return false;
 	             	}
 		...
 		return false;
			...
			function searchByCondition(type,value){
				...
				$("#order2").val('');
				if(verify!=1){
				document.formList.action="${ctx}/mobile/pages/publications/journalList";
  ...
  <div data-role="content">
    ...
    <div data-role="navbar" class="navDiv">
        <ul>
          ...
          <li class="curLi"><a href="${ctx}/mobile/pages/publications/journalList" data-ajax="false">期刊</a></li>
  	...
  	<form:form  id="form" commandName="form" action="${ctx}/mobile/pages/publications/journal">
    <!-- 中间内容部分开始 -->
    ...
    <div class="oh mb20" id="journalList">
    	...
    	<div class="booklist oh">
            ...
            <div class="fl wid85">
                <h1><a data-ajax="false" href="${ctx}/mobile/pages/publications/form/article/${b.id}" title="${b.title}">${fn:replace(fn:replace(fn:replace(b.title,"&lt;","<"),"&gt;",">"),"&amp;","&")}</a></h1>
               	<c:if test="${b.startVolume!=null && b.endVolume!=null }">
                <p><ingenta-tag:LanguageTag key="Pages.Publications.Journal.Lable.Volume" sessionKey="lang" /> ${b.startVolume }-<ingenta-tag:LanguageTag key="Pages.Publications.Journal.Lable.Volume" sessionKey="lang" /> ${b.endVolume }</p>
               </c:if>
                <p>${b.publisher.name}(${fn:substring(b.pubDate,0,4) })</p>
