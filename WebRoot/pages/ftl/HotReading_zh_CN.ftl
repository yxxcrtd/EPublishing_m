<h1 class="indexHtit">
	<span class="fl titFb"><a class="ico1">热读资源</a></span>
    <span class="fr"></span>
</h1>
		...
		<div class="block">
            ...
            <div class="fl w640">
				...
				<#if (p.publications.publisher.name!='')>
					<p><a href='${request.contextPath}/index/search?type=2&isAccurate=1&searchValue="${p.publications.publisher.name}"'>${p.publications.publisher.name}</a>
				...
				<#if (p.publications.type==2 && p.publications.startVolume!='' && p.publications.endVolume!='')>
					<p>
					卷 ${p.publications.startVolume }-卷 ${p.publications.endVolume }
					</p>
				</#if>
            </div>
        </div>
