...
<c:if test="${tocList!=null }">
	<c:forEach items="${tocList }" var="p" varStatus="index">
		...
		<li class=" ${p.fullText}" style="margin-left:${p.browsePrecent}px"><a   href="javascript:void(0)" onclick="$FlexPaper('documentViewer').gotoPage(${p.startPage==null?1:p.startPage})" title="${p.title }">${p.title }</a></li>
		</c:if>
		<c:if test="${p.startPage<=0}">
