...
<h1 class="indexHtit">
	...
	<c:forEach items="${list}" var="p" varStatus="index">
		...
		<div class="block">
            ...
            <div class="fl w640">
                <p><a class="a_title" href="${ctx}/mobile/pages/publications/form/article/${p.publications.id}" title="${p.publications.title}">${fn:replace(fn:replace(fn:replace(p.publications.title,"&lt;","<"),"&gt;",">"),"&amp;","&")}</a></p>
				...
				<c:if test="${not empty p.publications.publisher.name}">
					<p>${p.publications.publisher.name}<c:if test="${fn:substring(p.publications.pubDate,0,4)!=null }">(${fn:substring(p.publications.pubDate,0,4) })</c:if></p>
				</c:if>
