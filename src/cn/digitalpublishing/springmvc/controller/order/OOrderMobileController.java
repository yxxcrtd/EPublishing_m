...
@Controller
@RequestMapping("/mobile/pages/order")
public class OOrderMobileController extends BaseController {
	...
	public ModelAndView list(HttpServletRequest request,HttpServletResponse response,HttpSession session, OOrderForm form)throws Exception {
		String forwardString="mobile/order/list";
...
//			user.setId("086389937b1f1030a8ee2610e0325a2b");
	 ...
	 * @throws Exception
	 */
	@RequestMapping(value="/form/jsonList")
	public ModelAndView jsonList(HttpServletRequest request,HttpServletResponse response,HttpSession session, OOrderForm form )throws Exception {
		String forwardString="mobile/order/list";
		Map<String,Object> model = new HashMap<String,Object>();
		try{
			CUser user = session.getAttribute("mainUser")==null?null:(CUser)session.getAttribute("mainUser");
...
//			user.setId("086389937b1f1030a8ee2610e0325a2b");
			if(user!=null){
				...
				condition.put("endTime", endTime);
				form.setCount(this.oOrderService.getOrderCount(condition));
				if(form.getCurpage()==0){
					form.setCurpage(1);
				}
				List<OOrder> list = this.oOrderService.getOrderPagingList(condition," order by a.createdon desc ",20,form.getCurpage());
				form.setCurpage(form.getCurpage()+1);
				form.setAllStatus(condition.get("allStatus").toString());
				model.put("form", form);
				model.put("dr", dr);
				model.put("level", level);
				...
				for (OOrder oOrder : list) {
					oOrder.setCreatedonStr(fmt.format(oOrder.getCreatedon()));
				}
				map.put("list", list);
				map.put("curpage",String.valueOf(form.getCurpage()));
...
//			user.setId("086389937b1f1030a8ee2610e0325a2b");
	...
	public ModelAndView detail(HttpServletRequest request,HttpServletResponse response,HttpSession session, OOrderForm form)throws Exception {
		String forwardString="mobile/order/detailList";
		Map<String,Object> model = new HashMap<String,Object>();
		//总价格
		double tPrice=0d;
	...
	public void getOrderDetailList(@PathVariable String orderId,HttpServletRequest request,Model model)throws Exception {
		...
		try{
			...
			List<OOrderDetail> list = this.oOrderService.getDetailList(condition, null);
	...
	//			license.setEndTime(new Date());
				...
				log.setAlertDate(DateUtil.getDateStr("yyyy-MM-dd", date3));//提醒日期 提前一个月
		...
		try {
			...
			for (OOrder o : list2) {
				...
				for(OOrderDetail d : o.getDetailList()){	
					...
					switch (d.getStatus()) {
						case 1:  orderDetailStatus="未处理";			break;
						case 2:  orderDetailStatus="已付款未开通";		break;
						case 3:  orderDetailStatus="已付款已开通";		break;
						case 4:  orderDetailStatus="处理中";			break;
						case 5:  orderDetailStatus="已完成";			break;
						case 10:  orderDetailStatus="未付款已开通";	break;
						case 99:  orderDetailStatus="已取消";			break;	
						default:
							break;
					}
			...
			if(obj.getPublications()!=null){
				...
				if("2".equals(operType)){
					...
					if(pp.getStatus() ==2){//只有上架的图书、期刊、章节、文章、数据库才能增加全文索引
						if((pp.getLocal()!=null && pp.getLocal()==2) || (pp.getPublications().getLocal()!=null && pp.getPublications().getLocal()==2)){//只有本地资源才能增加全文索引
							String nr = "";
...
//				con.put("userId", user.getId());
//				List<LLicense> lList = this.pPublicationsService.getLicenseList(con, "");
				if("2".equals(operType)){
					if(license!=null){
						this.oOrderService.updateLicense(obj,obj.getId(),null);
					}
...
//			}
	...
	public ModelAndView alipaySubmit(HttpServletRequest request,HttpServletResponse response) throws Exception{
		...
		if(order!=null){
			...
			//String log_path = OOrderDetailForm.getAlipayMap().get("log_path");// 调试用，创建TXT日志文件夹路径
		
			String input_charset = OOrderDetailForm.getAlipayMap().get("input_charset");// 字符编码格式 目前支持 gbk 或 utf-8
			
			//String sign_type = OOrderDetailForm.getAlipayMap().get("sign_type");// 签名方式 不需修改
