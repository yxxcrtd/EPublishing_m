...
public class PCollectionController extends BaseController {
	...
	public ModelAndView manager(HttpServletRequest request, HttpServletResponse response, PCollectionForm form) throws Exception {
		...
		try {
			...
			// condition.put("unNullPrice", "打包集下面没产品，查出来干鸡毛？");
	...
	public ModelAndView list(HttpServletRequest request, HttpServletResponse response, PCollectionForm form) throws Exception {
		...
		try {
			...
			// 出版物 1-电子书 2-期刊 3-章节4-期刊期
			// 1 期刊
			condition.put("type", 2);
			model.put("magazineCount", this.pPublicationsService.getPccCount(condition));
			// 2 电子书
			condition.put("type", "1");
			model.put("bookCount", this.pPublicationsService.getPccCount(condition));
			// 2.1 中文电子书
			condition.put("lang", "CHS");
			model.put("cnBookCount", this.pPublicationsService.getPccCount(condition));
			condition.remove("lang");
			// 2.2 英文电子书
			condition.put("-lang", "CHS");
			model.put("enBookCount", this.pPublicationsService.getPccCount(condition));
			condition.remove("-lang");
			// 3. 文章
			condition.put("type", "4");
			model.put("articleCount", this.pPublicationsService.getPccCount(condition));
			condition.remove("type");
			// 4. 期刊期
			condition.put("type", "7");
			model.put("type7", this.pPublicationsService.getPccCount(condition));
			// 中文
			condition.put("lang", "CHS");
			model.put("cnCount", this.pPublicationsService.getPccCount(condition));
			// 英文
			condition.remove("lang");
			condition.put("-lang", "CHS");
			model.put("enCount", this.pPublicationsService.getPccCount(condition));
