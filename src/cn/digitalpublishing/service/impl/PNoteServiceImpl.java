...
public class PNoteServiceImpl extends BaseServiceImpl implements PNoteService {
	...
	@Override
	public PNote selectByConditon(Map<String, Object> condition) throws Exception {
		PNote note = null ; 
		List<PNote> list = null;
		try{
			list = this.daoFacade.getpNoteDao().getList(condition,null);
			note = null!=list && !list.isEmpty()? list.get(0) : null ; 
		}catch(Exception e){
			throw new CcsException((e instanceof CcsException) ? e.getMessage()	: "Note.info.pagelist.error", e);//查询笔记列表出错
		}
		return note;
	}
