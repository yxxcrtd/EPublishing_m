...
public class LLicenseDao extends CommonDao<LLicense, String> {
	...
	private Map<String,Object> getWhere(Map<String,Object> map){
		...
		/*if(CollectionsUtil.exist(map, "isTrail")&&!"".equals(map.get("isTrail")) && map.get("isTrail")!=null){
			if(flag==0){
		...
		if(CollectionsUtil.exist(map, "isTrail")&&"0".equals(map.get("isTrail"))){
			
		}else if(CollectionsUtil.exist(map, "isTrail")&&!"".equals(map.get("isTrail")) && map.get("isTrail")!=null){
				...
				condition.add(Integer.parseInt(map.get("isTrail").toString()));						
...
//		}
	...
	public List<LLicense> getList(Map<String,Object> condition,String sort)throws Exception{
		...
		String property="complicating,publications.listPrice,isTrial,trialPeriod,readUrl,id,code,createdby,createdon,status,type,startTime,endTime,licenseUId,licensePwd,accessType,accessUIdType,user.id,user.name,user.institution.id,user.institution.name,publications.id,publications.code,publications.title," +
				"publications.author,publications.remark,publications.type,publications.createOn,publications.pubDate,publications.pubSubject,publications.publisher.id,publications.publisher.name,publications.pubSubjectEn,publications.local" +
				",collection.id,collection.code,collection.name,collection.price,collection.currency,collection.createOn,collection.desc" +
				",publications.createDate";
		String field="a.complicating,d.listPrice,a.isTrial,a.trialPeriod,a.readUrl,a.id,a.code,a.createdby,a.createdon,a.status,a.type,a.startTime,a.endTime,a.licenseUId,a.licensePwd,a.accessType,a.accessUIdType,b.id,b.name,c.id,c.name,d.id,d.code,d.title,d.author,d.remark,d.type,d.createOn,d.pubDate,d.pubSubject,e.id,e.name,d.pubSubjectEn,d.local" +
				",col.id,col.code,col.name,col.price,col.currency,col.createOn,col.desc" +
				",d.createDate";
		try{
	...
	public List<LLicense> getPagingList(Map<String,Object> condition,String sort,Integer pageCount,Integer page)throws Exception{
		...
		Map<String,Object> t=this.getWhere(condition);
		String property="isTrial,trialPeriod,id,code,createdby,createdon,status,type,startTime,endTime,licenseUId,licensePwd,accessType,accessUIdType,user.id,user.name,publications.id,publications.code,publications.sisbn,publications.available,publications.title,publications.author,publications.pubDate,publications.cover,publications.publisher.name,publications.publisher.nameEn" +
				",collection.id,collection.code,collection.name,collection.price,collection.currency,collection.createOn,collection.desc";
		String field="a.isTrial,a.trialPeriod,a.id,a.code,a.createdby,a.createdon,a.status,a.type,a.startTime,a.endTime,a.licenseUId,a.licensePwd,a.accessType,a.accessUIdType,b.id,b.name,d.id,d.code,d.sisbn,d.available,d.title,d.author,d.pubDate,d.cover,e.name,e.nameEn" +
				",col.id,col.code,col.name,col.price,col.currency,col.createOn,col.desc";
		try{
	...
	public List<LLicense> getCartPagingListDown(Map<String,Object> condition,String sort,Integer pageCount,Integer page)throws Exception{
		...
		try {
			...
			//SELECT a.PUB_ID,count(*) as eachCount 
			    String hql="from LLicense a where EXISTS ("
					+ "select 1 from LLicense b where a.user.id=b.user.id AND EXISTS(SELECT 1 FROM PCsRelation c WHERE b.publications.id=c.publications.id AND EXISTS(SELECT 1 FROM BSubject d WHERE c.subject.id=d.id and d.code like '"+condition.get("mark")+"%'))"
					+")";
	...
	public Integer getResouceCount(Map<String,Object> condition)throws Exception{
		...
		String hql=" from LLicense a left join a.user b left join a.user.institution c left join a.publications d left join a.collection col";
		try{
			list=this.hibernateDao.getListByHql("id","cast(count(*) as string)", hql+t.get("where").toString().replace(" and (a.isTrial = 2 or a.isTrial is null)", ""), ((List<Object>)t.get("condition")).toArray(),"", LLicense.class.getName());
		}catch(Exception e){
			e.printStackTrace();
			throw e;
		}
	...
	public List<LLicense> getPagingListForIndex(Map<String,Object> condition,String sort,Integer pageCount,Integer page)throws Exception{
		...
		String hql=" from LLicense a left join a.user.institution c left join a.publications d left join d.publisher p";
		Map<String,Object> t=this.getWhere(condition);
		String property="publications.id,publications.startVolume,publications.endVolume,publications.pubDate,publications.author,publications.code,publications.title,publications.type,publications.sisbn,publications.hisbn," +
				"publications.remark,publications.cover,publications.publisher.id,publications.publisher.name ";
		String field="d.id,d.startVolume,d.endVolume,d.pubDate,d.author,d.code,d.title,d.type,d.sisbn,d.hisbn,d.remark,d.cover,p.id,p.name ";
		try{
			list=super.hibernateDao.getListByHql(property,field, hql+t.get("where").toString(), ((List<Object>)t.get("condition")).toArray(),sort, LLicense.class.getName(),pageCount,page*pageCount);
		}catch(Exception e){
			throw e;
	...
	public List<LLicense> getCanReadPagingList(Map<String,Object> condition,String sort,Integer pageCount,Integer page)throws Exception{
		...
		String property="isTrial,trialPeriod,id,code,createdby,createdon,status,type,startTime,endTime,licenseUId,licensePwd,accessType,accessUIdType,user.id,user.name,publications.id,publications.code,publications.available,publications.title,publications.startVolume,publications.endVolume,publications.publisher.name" +
				",collection.id,collection.code,collection.name,collection.price,collection.currency,collection.createOn,collection.desc";
		String field="a.isTrial,a.trialPeriod,a.id,a.code,a.createdby,a.createdon,a.status,a.type,a.startTime,a.endTime,a.licenseUId,a.licensePwd,a.accessType,a.accessUIdType,b.id,b.name,d.id,d.code,d.available,d.title,d.startVolume,d.endVolume,e.name" +
				",col.id,col.code,col.name,col.price,col.currency,col.createOn,col.desc";
		try{
			list=super.hibernateDao.getListByHql(property,field, hql+t.get("where").toString(), ((List<Object>)t.get("condition")).toArray(),sort, LLicense.class.getName(),pageCount,page*pageCount);
		}catch(Exception e){
