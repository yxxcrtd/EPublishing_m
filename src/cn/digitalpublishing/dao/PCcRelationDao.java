...
public class PCcRelationDao extends CommonDao<PCcRelation, String> {
	...
	public List<PCcRelation> getPagingList(Map<String, Object> condition, String sort, Integer pageCount, Integer page, CUser user, String ip) throws Exception {
		...
		String property = "publications.activity,publications.volume.id,publications.issue.id,publications.year,publications.month,publications.volumeCode,publications.issueCode,publications.chineseTitle,publications.sisbn,publications.hisbn" + ",collection.currency,collection.price,id,collection.id,collection.desc,collection.name,publications.id,publications.title" + ",publications.authorAlias,publications.author,publications.code,publications.cover,publications.pubDate" + ",publications.publisherName,publications.publisherId" + ",publications.type" + ",publications.startVolume" + ",publications.endVolume" + ",publications.oa,publications.free,publications.inCollection";
		if (user != null) {
			property += ",publications.subscribedIp" + ",publications.subscribedUser" + ",publications.favorite";
		}
		if (CollectionsUtil.exist(condition, "ins_id") || !"".equals(condition.get("ins_id"))) {
			property += ",publications.inLicense" + ",publications.inDetail" + ",publications.insfavorite";
		}
		property += ",publications.recommand";
		String field = "p.activity,p.volume.id,p.issue.id,p.year,p.month,p.volumeCode,p.issueCode,p.chineseTitle,p.sisbn,p.hisbn" + ",c.currency,c.price,a.id,c.id,c.desc,c.name,p.id,p.title" + ",p.author,p.author,p.code,p.cover,p.pubDate" + ",(select pp.publisher.name from PPublications pp left join pp.publisher where pp.id=p.id)" + ",(select pp.publisher.id from PPublications pp left join pp.publisher where pp.id=p.id)" + ",p.type" + ",p.startVolume" + ",p.endVolume" + ",p.oa,p.free" + ",'1'";
		if (user != null) {
	...
	public List<PCcRelation> getList(Map<String, Object> condition, String sort, CUser user, String ip) throws Exception {
		...
		String property = "complicating,effective,id,collection.id,collection.desc,collection.name,publications.id,publications.title" + ",publications.author,publications.code,publications.cover,publications.pubDate" + ",publications.publisherName,publications.publisherId,publications.subjectName,publications.type" + ",publications.oa,publications.free,publications.inCollection,publications.latest,publications.subscribedIp";
		if (user != null) {
			property += ",publications.subscribedUser,publications.exLicense,publications.buyInDetail,publications.favorite";
		}
		property += ",publications.recommand";
		String field = "a.complicating,a.effective,a.id,c.id,c.desc,c.name,p.id,p.title" + ",p.author,p.code,p.cover,p.pubDate" + ",(select pp.publisher.name from PPublications pp left join pp.publisher where pp.id=p.id)" + ",(select pp.publisher.id from PPublications pp left join pp.publisher where pp.id=p.id)" + ",p.pubSubject" + ",p.type,p.oa,p.free" + ",'1'" + ",(select cast(count(*) as string) from PPublications pub where pub.createOn>=? and pub.id=p.id)" + ",(select cast(count(*) as string) from LLicenseIp lip where lip.license.status=1 and lip.sip<=? and lip.eip>=? and lip.license.publications.id=a.id )";
		if (user != null) {
