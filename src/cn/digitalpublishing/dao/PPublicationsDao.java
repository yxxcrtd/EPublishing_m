...
public class PPublicationsDao extends CommonDao<PPublications, String> {
	...
	private Map<String, Object> getWhere(Map<String, Object> map) {
		...
		if ((CollectionsUtil.exist(map, "UID")) && map.get("UID") != null && !"".equals(map.get("UID"))) {
			if (flag == 0) {
				whereString = whereString + " where c.id= ? ";
				flag = 1;
			} else {
				whereString = whereString + " and  c.id = ?  ";
			}
			condition.add(map.get("UID").toString());
		}
		if ((CollectionsUtil.exist(map, "ppStatus")) && map.get("ppStatus") != null && !"".equals(map.get("ppStatus"))) {
			if (flag == 0) {
				whereString = whereString + " where d.status= ? ";
				flag = 1;
			} else {
				whereString = whereString + " and  d.status = ?  ";
			}
			condition.add(Integer.valueOf(map.get("ppStatus").toString()));
		}
		...
		if (CollectionsUtil.exist(map, "publisher") && !"".equals(map.get("publisher")) && map.get("publisher") != null) {
			...
			if (flag == 0) {
				whereString += " where b.name is not null ";
				flag = 1;
			} else {
				whereString += " and b.name is not null ";
			}
		...
		if (CollectionsUtil.exist(map, "createOn") && !"".equals(map.get("createOn")) && null != map.get("createOn")) {
			if (0 == flag) {
				whereString += " WHERE a.createOn BETWEEN TO_DATE('" + DateUtil.getMonthStartDay(new Date()) + "', 'yyyy-MM-dd') AND TO_DATE(?, 'yyyy-MM-dd')";
				flag = 1;
			} else {
				whereString += " AND a.createOn BETWEEN TO_DATE('" + DateUtil.getMonthStartDay(new Date()) + "', 'yyyy-MM-dd') AND TO_DATE(?, 'yyyy-MM-dd')";
			}
			condition.add(map.get("createOn"));
		}

		// 期号不为空
		if (CollectionsUtil.exist(map, "issueNotNull")) {

			if (flag == 0) {
				whereString += " where a.issue is not null ";
				flag = 1;
			} else {
				whereString += " and a.issue is not null ";
			}
			// condition.add(map.get("issue"));

		}

		if (CollectionsUtil.exist(map, "issueCode") && !"".equals(map.get("volumeCode")) && null != map.get("volumeCode")) {
			if (0 == flag) {
				whereString += " WHERE a.local = ? ";
				flag = 1;
			} else {
		...
		if (CollectionsUtil.exist(map, "volumeCode") && !"".equals(map.get("volumeCode")) && null != map.get("volumeCode")) {
			if (0 == flag) {
				whereString += " WHERE a.PUB_VOLUME = ? ";
				flag = 1;
			} else {
				whereString += " AND a.PUB_VOLUME = ? ";
			}
			condition.add(map.get("local"));
		}
	...
	public List<PPublications> getList(Map<String, Object> condition, String sort, CUser user, String ip) throws Exception {
		...
		",a.inCollection" + ",(case when exists(select pub.id from PPublications pub where pub.createOn >=? and pub.id=a.id) then '1' else '0' end) as isnewst" +
				// ",'0'";
				",(case when exists(select lip.id from LLicenseIp lip where (lip.license.publications.id=a.id or lip.license.publications.code=a.code) and lip.sip<=? and lip.eip>=? and lip.license.status=1) then '1' else '0' end) as IPRange" + ",c.id " + ",c.title " + ",c.pubSubject";
		if (user != null) {
	...
	public List<PPublications> getpublicationsPagingList(Map<String, Object> condition, String sort, Integer pageCount, Integer curPage) throws Exception {
		...
		// 求当前时间前的2周以前的日期
		Map<String, Object> t = this.getWhere(condition);
		String property = " id,title,cover ,type,free,oa,author,pubDate,publisher.name,startVolume,endVolume";
		String field = " a.id,a.title,a.cover,a.type,a.oa,a.free,a.author,a.pubDate,b.name,a.startVolume,a.endVolume";
		try {
	...
	public List<PPublications> getSimplePubList(Map<String, Object> condition, String sort, Integer number) throws Exception {
		...
		if (condition.containsKey("justYear")) {
			...
			property = "year,volumeCode,month,issue.id";
			field = "a.year,a.volumeCode,a.month,a.issue.id";
		} else {
	...
	public List<PPublications> getpublisherInfoForCol(Map<String, Object> condition, String sort) throws Exception {
		...
		String hql = " from PPublications a where exists(select 1 from PCcRelation b where a.id=b.publications.id and b.collection.id='" + condition.get("collectionId") + "') group by a.publisher.id ";
		try {
	...
	public List<PPublications> getLangInfoForCol(Map<String, Object> condition, String sort) throws Exception {
		...
		String hql = " from PPublications a where exists(select 1 from PCcRelation b where a.id=b.publications.id and b.collection.id='" + condition.get("collectionId") + "') group by a.lang ";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql, ((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName());
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<PPublications> getPublications(Map<String, Object> condition, String sort, Integer pageCount, Integer curPage) throws Exception {
		List<PPublications> list = null;
		String hql = " from PPublications a left join a.publisher b left join a.publications p ";
		// 求当前时间前的2周以前的日期
		Map<String, Object> t = this.getWhere(condition);
	 ...
	 * @throws Exception
	 */
	public List<PPublications> getSubPagingList(Map<String, Object> condition, String sort, Integer pageCount, Integer page) throws Exception {
		List<PPublications> list = null;
		Map<String, Object> t = this.getWhere(condition);
		String hql = " from LLicense a left join a.user c left join a.publications d left join d.publisher e ";
		String property = " id,title,type,author,publisher.name";
		String field = " d.id,d.title,d.type,d.author,e.name";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), ((List<Object>) t.get("condition")).toArray(), sort, PPublications.class.getName(), pageCount, page * pageCount);
		} catch (Exception e) {
			throw e;
