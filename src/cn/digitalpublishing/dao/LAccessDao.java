...
public class LAccessDao extends CommonDao<LAccess, String> {
	...
	public List<LAccess> getJournalHotReading(Map<String, Object> condition, String sort, int pageCount, int curPage) throws Exception {
		...
		String hql = " from LAccess a ";
		Map<String, Object> t = this.getWhere(condition);
		String property = "publications.id,publications.type,publications.author,publications.pubDate,publications.title,publications.code,publications.remark,publications.publisher.name,publications.publisher.id";
		String field = " " + "a.publications.id" + ",(select p.type from PPublications p where p.id=a.publications.id)" + ",(select p.author from PPublications p where p.id=a.publications.id )" + ",(select p.pubDate from PPublications p where p.id=a.publications.id )" + ",(select p.title from PPublications p where p.id=a.publications.id )" + ",(select p.code from PPublications p where p.id=a.publications.id )" + ",(select p.remark from PPublications p where p.id=a.publications.id )" + ",(select pp.name from PPublications p left join p.publisher pp where p.id=a.publications.id )" + ",(select pp.id from PPublications p left join p.publisher pp where p.id=a.publications.id )";
		try {
			list = super.hibernateDao.getListByHql(property, field, hql + t.get("where").toString(), ((List<Object>) t.get("condition")).toArray(), sort, LAccess.class.getName(), pageCount, curPage * pageCount);
		} catch (Exception e) {
			e.printStackTrace();
